...
**Persist the three completion reports to disk, not only to chat.** The `## Swagger Verification Report` goes to `.ai/artifacts/swagger-verification.md`, the `## Security Report` to `.ai/artifacts/security-report.md`, and the generated API documentation to the project's docs location (`docs/api/api-documentation.md` by default). In the `prd-to-prod` workflow this skill runs as Stage 8 (Implementation), and `.ai/artifacts/` is that workflow's canonical artifact location. Both reports are **enforced there, not merely expected**: `hooks/stop.js` refuses to end the turn until `swagger-verification.md` and `security-report.md` both exist, and `hooks/pre-tool.js` denies either write unless it carries its status line with a passing value — `Validation status: PASS` (or `N/A — no REST endpoints`) for Swagger, `Final status: PASS` (or `PASS WITH USER-ACCEPTED FINDINGS`) for security. A `FAIL` status is denied at write time, because a failing gate recorded as a completed task is the exact failure this exists to prevent. A report written only into the conversation is therefore not a shortcut, it is a blocked turn. A task with no HTTP surface still writes `swagger-verification.md`, stating `Validation status: N/A — no REST endpoints` and why; that satisfies the gate and waives the API-documentation requirement, and nothing waives the Security Report.
